...
if ($op == 'ADD') {
    ...
    if ($tblType == "Report") {//DOES NOT WORK
       ...
       // $PRReportNote=filter_input(INPUT_POST, 'PRReportNote');
        //$PRReportNote = $connection->sanitize($PRReportNote);/////////////////
...
VALUES ('$fkPlacementID', '$PREdSchoolType', '$PREdStatus', '$PREdStartDate', '$PREdEndDate', '$IsPREdFullTime', '$PREdNote' )";
    ...
    {
        $fkPlacementID = filter_input(INPUT_POST,'fkPlacementID',FILTER_SANITIZE_NUMBER_INT);
       // $PREmployer = $connection->sanitize($_POST['PREmployer']);
        $PREmployer = filter_input(INPUT_POST, 'PREmployer', FILTER_SANITIZE_ENCODED);
